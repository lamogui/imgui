...
static void ImGui_ImplDX11_CreateWindow(ImGuiViewport* viewport)
{
    ...
    eMemSet(&sd, 0, sizeof(sd));
    sd.BufferDesc.Width = eFtoL(viewport->Size.x);
    sd.BufferDesc.Height = eFtoL(viewport->Size.y);
...
static void ImGui_ImplDX11_SetWindowSize(ImGuiViewport* viewport, ImVec2 size)
{
    ...
    if (data->SwapChain)
    {
        ...
        data->SwapChain->ResizeBuffers(0, eFtoL(size.x), eFtoL(size.y), DXGI_FORMAT_UNKNOWN, 0);
        data->SwapChain->GetBuffer(0, IID_PPV_ARGS(&pBackBuffer));
        if (pBackBuffer == NULL) { lab::Error( "ImGui_ImplDX11_SetWindowSize() failed creating buffers.\n" ); return; }
        g_pd3dDevice->CreateRenderTargetView(pBackBuffer, NULL, &data->RTView);
        pBackBuffer->Release();
    }
}
