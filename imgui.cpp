...
{
    int d;
    while ((d = eToUpper(*str2) - eToUpper(*str1)) == 0 && *str1) { str1++; str2++; }
    return d;
}
...
{
    int d = 0;
    while (count > 0 && (d = eToUpper(*str2) - eToUpper(*str1)) == 0 && *str1) { str1++; str2++; count--; }
    return d;
}
...
void ImStrncpy(char* dst, const char* src, size_t count)
{
    if (count < 1)
        return;
    if (count > 1)
        eStrNCopy(dst, src, count - 1);
    dst[count - 1] = 0;
}

char* ImStrdup(const char* str)
{
    size_t len = eStrLength(str);
    void* buf = IM_ALLOC(len + 1);
    eMemCopy( buf, ( const void* )str, len + 1 );
    return ( char* )buf;
}

char* ImStrdupcpy(char* dst, size_t* p_dst_size, const char* src)
{
    size_t dst_buf_size = p_dst_size ? *p_dst_size : eStrLength(dst) + 1;
    size_t src_size = eStrLength(src) + 1;
    ...
    eMemCopy( dst, ( const void* )src, src_size );
    return (char*)dst;
}

const char* ImStrchrRange(const char* str, const char* str_end, char c)
{
    const char* p = (const char*)eMemChr(str, (int)c, str_end - str);
    return p;
}
...
{
    const char* p = (const char*)eMemChr(str, '\n', str_end - str);
    return p ? p : str_end;
}
...
{
    if (!needle_end)
        needle_end = needle + eStrLength(needle);

    const char un0 = (char)eToUpper(*needle);
    while ((!haystack_end && *haystack) || (haystack_end && haystack < haystack_end))
    {
        if ( eToUpper(*haystack) == un0)
        {
            const char* b = needle + 1;
            for (const char* a = haystack + 1; b < needle_end; a++, b++)
                if ( eToUpper(*a) != eToUpper(*b))
                    break;
            if (b == needle_end)
                return haystack;
        }
...
void ImStrTrimBlanks(char* buf)
{
    ...
    if (p_start != buf)                     // Copy memory if we had leading blanks
        eMemMove(buf, p_start, p - p_start);
...
#else
    int w = prout_vsnprintf(buf, buf_size, fmt, args);
#endif
    va_end(args);
    if (buf == NULL)
        return w;
...
void*   ImFileLoadToMemory(const char* filename, const char* mode, size_t* out_file_size, int padding_bytes)
{
    ...
    if (padding_bytes > 0)
        eMemSet((void*)(((char*)file_data) + file_size), 0, (size_t)padding_bytes);
...
{
    Filters.resize(0);
    ImGuiTextRange input_range(InputBuf, InputBuf+ eStrLength(InputBuf));
...
{
    int len = str_end ? (int)(str_end - str) : (int)eStrLength(str);
    ...
    eMemCopy(&Buf[write_off - 1], str, (size_t)len);
...
ImU32 ImGui::GetColorU32(ImU32 col)
{
    ...
    a = eFtoL( static_cast< float >( a ) * style.Alpha); // We don't need to clamp 0..255 because Style.Alpha is in 0..1 range.
...
void ImGui::RenderText(ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash)
{
    ...
    {
        if (!text_end)
            text_end = text + eStrLength(text); // FIXME-OPT
    ...
    : DrawListInst(&context->DrawListSharedData)
{
    ...
    NameBufLen = (int)eStrLength(name) + 1;
    MoveId = GetID("#MOVE");
    ChildId = 0;
...
void ImGui::NewFrame()
{
    ...
    eMemCopy(g.IO.KeysDownDurationPrev, g.IO.KeysDownDuration, sizeof(g.IO.KeysDownDuration));
...
void ImDrawDataBuilder::FlattenIntoSingleLayer()
{
    ...
    for (int layer_n = 1; layer_n < IM_ARRAYSIZE(Layers); layer_n++)
    {
        ...
        if (layer.empty())
            continue;
        eMemCopy(&Layers[0][n], &layer[0], layer.Size * sizeof(ImDrawList*));
        n += layer.Size;
        layer.resize(0);
    }
}
...
void ImGui::EndFrame()
{
    ...
    eMemSet(g.IO.NavInputs, 0, sizeof(g.IO.NavInputs));
}
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        if ((window->Viewport && window->Viewport->Window == window) || (window->DockIsActive))
            ...
            window_title_visible_elsewhere = true;
        if (window_title_visible_elsewhere && !window_just_created && eStrCompare(name, window->Name) != 0)
        {
        ...
        if (window->ViewportOwned)
        {
            ...
            else if (eMemCompare(&window->Viewport->Pos, &window->Pos, sizeof(window->Pos)) != 0)
            {
            ...
            else if (eMemCompare(&window->Viewport->Size, &window->Size, sizeof(window->Size)) != 0)
            {
                viewport_rect_changed = true;
...
void ImGui::BringWindowToFocusFront(ImGuiWindow* window)
{
    ...
    for (int i = g.WindowsFocusOrder.Size - 2; i >= 0; i--) // We can ignore the top-most window
        ...
        {
            eMemMove(&g.WindowsFocusOrder[i], &g.WindowsFocusOrder[i + 1], (size_t)(g.WindowsFocusOrder.Size - i - 1) * sizeof(ImGuiWindow*));
...
void ImGui::BringWindowToDisplayFront(ImGuiWindow* window)
{
    ...
    for (int i = g.Windows.Size - 2; i >= 0; i--) // We can ignore the top-most window
        ...
        {
            eMemMove(&g.Windows[i], &g.Windows[i + 1], (size_t)(g.Windows.Size - i - 1) * sizeof(ImGuiWindow*));
...
void ImGui::BringWindowToDisplayBack(ImGuiWindow* window)
{
    ...
    for (int i = 0; i < g.Windows.Size; i++)
        if (g.Windows[i] == window)
        {
            eMemMove(&g.Windows[1], &g.Windows[0], (size_t)i * sizeof(ImGuiWindow*));
            g.Windows[0] = window;
            break;
        }
}
...
{
    bool error = false;
    if (eStrCompare(version, IMGUI_VERSION) != 0) { error = true; IM_ASSERT(eStrCompare(version, IMGUI_VERSION) == 0 && "Mismatched version string!"); }
...
static void ImGui::NavUpdate()
{
    ...
    eMemCopy(g.IO.NavInputsDownDurationPrev, g.IO.NavInputsDownDuration, sizeof(g.IO.NavInputsDownDuration));
...
static const char* GetFallbackWindowNameForWindowingList(ImGuiWindow* window)
{
    if (window->Flags & ImGuiWindowFlags_Popup)
        return "(Popup)";
    if ((window->Flags & ImGuiWindowFlags_MenuBar) && eStrCompare(window->Name, "##MainMenuBar") == 0)
...
void ImGui::ClearDragDrop()
{
    ...
    eMemSet(&g.DragDropPayloadBufLocal, 0, sizeof(g.DragDropPayloadBufLocal));
}
...
bool ImGui::SetDragDropPayload(const char* type, const void* data, size_t data_size, ImGuiCond cond)
{
    ...
    IM_ASSERT(type != NULL);
    IM_ASSERT( eStrLength(type) < IM_ARRAYSIZE(payload.DataType) && "Payload type can be at most 32 characters long");
    ...
    if (cond == ImGuiCond_Always || payload.DataFrameCount == -1)
    {
        ...
        if (data_size > sizeof(g.DragDropPayloadBufLocal))
        {
            ...
            eMemCopy(payload.Data, data, data_size);
        }
        else if (data_size > 0)
        {
            // Store locally
...
ImGuiWindowSettings* ImGui::CreateNewWindowSettings(const char* name)
{
    ...
    if (const char* p = eStrStr(name, "###"))
        name = p;
#endif
    const size_t name_len = eStrLength(name);
    ...
    eMemCopy(settings->GetName(), name, name_len + 1);   // Store with zero terminator
...
void ImGui::LoadIniSettingsFromMemory(const char* ini_data, size_t ini_size)
{
    ...
    if (ini_size == 0)
        ini_size = eStrLength(ini_data);
    char* buf = (char*)IM_ALLOC(ini_size + 1);
    char* buf_end = buf + ini_size;
    eMemCopy(buf, ini_data, ini_size);
    buf[ini_size] = 0;
...
static void WindowSettingsHandler_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line)
{
    ...
    int i;
    ImU32 u1;
    if (prout_sscanf(line, "Pos=%i,%i", &x, &y) == 2)                 { settings->Pos = ImVec2ih((short)x, (short)y); }
    else if (prout_sscanf(line, "Size=%i,%i", &x, &y) == 2)           { settings->Size = ImVec2ih((short)x, (short)y); }
    else if (prout_sscanf(line, "ViewportId=0x%08X", &u1) == 1)       { settings->ViewportId = u1; }
    else if (prout_sscanf(line, "ViewportPos=%i,%i", &x, &y) == 2)    { settings->ViewportPos = ImVec2ih((short)x, (short)y); }
    else if (prout_sscanf(line, "Collapsed=%d", &i) == 1)             { settings->Collapsed = (i != 0); }
    else if (prout_sscanf(line, "DockId=0x%X,%d", &u1, &i) == 2)      { settings->DockId = u1; settings->DockOrder = (short)i; }
    else if (prout_sscanf(line, "DockId=0x%X", &u1) == 1)             { settings->DockId = u1; settings->DockOrder = -1; }
    else if (prout_sscanf(line, "ClassId=0x%X", &u1) == 1)            { settings->ClassId = u1; }
}
...
static void ImGui::DockSettingsHandler_ReadLine(ImGuiContext* ctx, ImGuiSettingsHandler*, void*, const char* line)
{
    ...
    if      (eStrNCompare(line, "DockNode", 8) == 0)  { line = ImStrSkipBlank(line + eStrLength("DockNode")); }
    else if (eStrNCompare(line, "DockSpace", 9) == 0) { line = ImStrSkipBlank(line + eStrLength("DockSpace")); node.Flags |= ImGuiDockNodeFlags_DockSpace; }
    else return;
    if (prout_sscanf(line, "ID=0x%08X%n",      &node.ID, &r) == 1)            { line += r; } else return;
    if (prout_sscanf(line, " Parent=0x%08X%n", &node.ParentNodeId, &r) == 1)  { line += r; if (node.ParentNodeId == 0) return; }
    if (prout_sscanf(line, " Window=0x%08X%n", &node.ParentWindowId, &r) ==1) { line += r; if (node.ParentWindowId == 0) return; }
    if (node.ParentNodeId == 0)
    {
        if (prout_sscanf(line, " Pos=%i,%i%n",  &x, &y, &r) == 2)         { line += r; node.Pos = ImVec2ih((short)x, (short)y); } else return;
        if (prout_sscanf(line, " Size=%i,%i%n", &x, &y, &r) == 2)         { line += r; node.Size = ImVec2ih((short)x, (short)y); } else return;
    }
    else
    {
        if (prout_sscanf(line, " SizeRef=%i,%i%n", &x, &y, &r) == 2)      { line += r; node.SizeRef = ImVec2ih((short)x, (short)y); }
    }
    if (prout_sscanf(line, " Split=%c%n", &c, &r) == 1)                   { line += r; if (c == 'X') node.SplitAxis = ImGuiAxis_X; else if (c == 'Y') node.SplitAxis = ImGuiAxis_Y; }
    if (prout_sscanf(line, " NoResize=%d%n", &x, &r) == 1)                { line += r; if (x != 0) node.Flags |= ImGuiDockNodeFlags_NoResize; }
    if (prout_sscanf(line, " CentralNode=%d%n", &x, &r) == 1)             { line += r; if (x != 0) node.Flags |= ImGuiDockNodeFlags_CentralNode; }
    if (prout_sscanf(line, " NoTabBar=%d%n", &x, &r) == 1)                { line += r; if (x != 0) node.Flags |= ImGuiDockNodeFlags_NoTabBar; }
    if (prout_sscanf(line, " HiddenTabBar=%d%n", &x, &r) == 1)            { line += r; if (x != 0) node.Flags |= ImGuiDockNodeFlags_HiddenTabBar; }
    if (prout_sscanf(line, " NoWindowMenuButton=%d%n", &x, &r) == 1)      { line += r; if (x != 0) node.Flags |= ImGuiDockNodeFlags_NoWindowMenuButton; }
    if (prout_sscanf(line, " NoCloseButton=%d%n", &x, &r) == 1)           { line += r; if (x != 0) node.Flags |= ImGuiDockNodeFlags_NoCloseButton; }
    if (prout_sscanf(line, " Selected=0x%08X%n", &node.SelectedWindowId,&r) == 1) { line += r; }
    ImGuiDockContext* dc = ctx->DockContext;
    if (node.ParentNodeId != 0)
...
static void SetClipboardTextFn_DefaultImpl(void*, const char* text)
{
    ...
    CFDataRef cf_data = CFDataCreate(kCFAllocatorDefault, (const UInt8*)text, eStrLength(text));
    if (cf_data)
    {
...
static void SetClipboardTextFn_DefaultImpl(void*, const char* text)
{
    ImGuiContext& g = *GImGui;
    g.ClipboardHandlerData.clear();
    const char* text_end = text + eStrLength(text);
    g.ClipboardHandlerData.resize((int)(text_end - text) + 1);
    eMemCopy(&g.ClipboardHandlerData[0], text, (size_t)(text_end - text));
